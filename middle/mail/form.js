...
const form = {
  mailForm: (mailHash, timeHash, form) => {
    const mail = {
      subject: "[VTS] 회원가입 인증",
      TEST: `<div><a href="http://${process.env.SERVER_HOST}/auth/email?id=${timeHash}"><img src="https://www.saraminimage.co.kr?url=http://www.vreducation.kr/img/20210401_saramin.png"></a></div>`,
      html: `
      <div style="font-family:'Malgun Gothic', sans-serif; padding:20px;">
        ...
        <a href="http://${process.env.SERVER_HOST}:19082/auth/email?id=${timeHash}" style="font-family:'Malgun Gothic', sans-serif; text-decoration: none;color: #fff;background: #5f63ff;display: inline-block;font-size: 15px;margin: 15px 0 50px;padding: 8px 15px;border-radius: 5px;">이메일 인증</a>
        ...
        <a href="#" style="font-family:'Malgun Gothic', sans-serif; font-size: 15px; margin: 0;color: #929292;">http://${process.env.SERVER_HOST}:19082/auth/email?id=${timeHash}</a>
    ...
    const Fails = {
      ...
      TEST: `<div><a href="http://${process.env.SERVER_HOST}:19082/m/auth/reset-password/${mailHash}/${timeHash}"><img src="https://www.saraminimage.co.kr?url=http://www.vreducation.kr/img/20210401_saramin.png"></a></div>`,
      html: `
      <div style="font-family:'Malgun Gothic', sans-serif; padding:20px;">
        <h1 style="font-family:'Malgun Gothic', sans-serif; font-size: 23px;margin: 0;font-weight:bold;">Enjoy Street</h1>
        ...
        <a href="http://${process.env.SERVER_HOST}:19082/m/auth/reset-password/${mailHash}/${timeHash}" style="font-family:'Malgun Gothic', sans-serif; text-decoration: none;color: #fff;background: #5f63ff;display: inline-block;font-size: 15px;margin: 15px 0 50px;padding: 8px 15px;border-radius: 5px;">비밀번호 재설정하기</a>
        <p style="font-family:'Malgun Gothic', sans-serif; margin: 0;display: block;font-size: 15px;">감사합니다.</p>
        <p style="font-family:'Malgun Gothic', sans-serif; margin: 0;display: block;font-size: 15px;">ENJOY STREET 드림</p>
        <div style="font-family:'Malgun Gothic', sans-serif; display: block;width: 100%;margin: 15px 0 0;height: 1px;background: #ccc;"></div>
        <p style="font-family:'Malgun Gothic', sans-serif; font-size: 15px;margin: 10px 0 0;color: #929292;">만약 버튼이 정상적으로 클릭되지 않는다면, 아래의 링크를 복사해서 접속해 주세요.</p>
        <a href="#" style="font-family:'Malgun Gothic', sans-serif; font-size: 15px; margin: 0;color: #929292;">http://${process.env.SERVER_HOST}:19082/m/auth/reset-password/${mailHash}/${timeHash}</a>
        <p style="font-family:'Malgun Gothic', sans-serif; margin-top: 50px;font-size: 12px;margin: 50px 0 0;">본 메일은 발신전용 메일이며 문의에 대해 회신 되지 않습니다.</p>
        <p style="font-family:'Malgun Gothic', sans-serif; margin: 40px 0 0;font-size: 12px;">주식회사 브이알에듀&nbsp; |  대표 김재헌&nbsp; |  사업등록번호 485-88-00534</p>
        <p style="font-family:'Malgun Gothic', sans-serif; margin: 0;font-size: 12px;">개인정보보호 책임자 김재헌&nbsp; |  통신판매업신고번호 제2020-서울강남-03050호&nbsp;관광사업자  |  등록번호 제2021-07호</p>
        <p style="font-family:'Malgun Gothic', sans-serif; margin: 0;font-size: 12px;">04542 서울특별시 중구 청계천로 100 시그니쳐타워 9층 932호</p>
    ...
    const dormant = {
      subject: "[VTS] 휴먼계정 인증",
      html: `<div><a href="http://${process.env.SERVER_HOST}:19082/auth/email?id=${timeHash}"><img src="https://www.saraminimage.co.kr?url=http://www.vreducation.kr/img/20210401_saramin.png"></a></div>`,
    };
    const store = {
