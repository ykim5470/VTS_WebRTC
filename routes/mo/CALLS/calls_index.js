...
const { isLoggedIn, isStatusCheck, isLoggedInGuide, isConfirmCheck, diviceCheck } = require("../../../middle/certification/middlewares");
...
router.get('/l', isLoggedIn, calls.output.list)
router.get('/u/rec', calls.output.recView)
router.get('/u/l', calls.output.userList)

 // ./public/all.js에서 post 된 payload 필요
router.post("/broadcast", isLoggedIn, calls.process.streamer);
router.post("/consumer", calls.process.viewer);
router.post('/l/activeOn', isLoggedIn, calls.process.recUploads)
